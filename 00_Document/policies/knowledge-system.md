# Knowledge System — AI 캐시 도메인별 _index + GC + 트랙 B 분리
...
본 문서는 SubAgent가 도메인별 학습을 *축적·조회*하는 Knowledge 시스템과, 학습 일지(트랙 B)와의 *경계*, 그리고 *GC Collector* 정책을 정의합니다. 디테일은 Phase 04 산출물 ([`../knowledge/`](../knowledge/)).
...
5/20 의논 결과 — KPI 전환("학습 박제 중심" → "Planning → 구현 → 보고")으로 *AI 활용 캐시*와 *본인 회고 일지*를 분리:
...
**경계 명확화**: 트랙 A는 *코드 패턴·도메인 결함·재현 시나리오*. 트랙 B는 *왜 그렇게 결정했나·내가 무엇을 배웠나*. 가짜 학습 방지를 위해 분리 (ADR-013 정신).
...
## 6. 트랙 A ↔ 트랙 B 경계 (가짜 학습 방지)

**트랙 A**(Knowledge) = AI가 직접 활용. 구조화·재현 가능·검증된 패턴.
**트랙 B**(학습 일지) = 본인 회고. AI 활용 안 함. 면접 무기.

### 양쪽 모두 박을 가치 있는 경우

같은 사건이 트랙 A와 B 양쪽에 박힐 수 있음 — 단 *시각*이 다름:

- 트랙 A: *어떻게 검출하나 / 어떻게 막나* (패턴 구조)
- 트랙 B: *내가 무엇을 배웠나 / 왜 그렇게 결정했나* (회고)

### 한쪽만 박을 경우

- *도메인 패턴 100%, 회고 0%* → 트랙 A만 (예: `PacketGenerator noManager 기본값`)
- *회고 100%, 패턴 0%* → 트랙 B만 (예: γ 방식 도입 결정 회고)
