...
| 2026-06-08 | ADR-030 신규 (행동 상태 규칙 = 서버 권위, 클라 Animator Exit Time = 시각 거울) | M4.6 ActionState FSM 진입 시 채택. 헌법 #1의 행동·애니 영역 구체 적용 — 공격 중 이동 잠금(commit window)을 서버 State에 강제, 클라 Exit Time은 거울일 뿐. M4.5 telegraph 상수 + M4.6 commit window로 같은 원칙 2회 반복 → 아키텍처 원칙 박제(plan-auditor 라우팅 제안 발화). 규칙 수치는 98_Shared 단일 상수(클라 예측 일치 → reconcile rubber-band 방지). 시각 전용 효과는 원칙 밖. gameplay/ 카테고리(5개째). |
