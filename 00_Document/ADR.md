...
- **[ADR/harness/](ADR/harness/)** — 작업 흐름·문서·훅 (6개: 013 014 015 016 018 020)
...
> 본인이 진행하다가 다음 결정들을 할 때 ADR로 기록하세요. 번호는 채택 순서대로 부여 — 아래는 가이드일 뿐. 채택분(ADR-020 = 훅 환경 의존성, 2026-05-14)은 후보 표에서 제거됨.

- **ADR-019**: 인증 방식 (단순 닉네임 → JWT? 세션?)
- **ADR-021**: 캐릭터 데이터 스키마 (정규화 vs JSONB)
- **ADR-022**: 채팅 시스템 (TCP로 전송 vs 별도 채널)
- **ADR-023**: 로그 저장 (로컬 파일 vs 외부 sink)
- **ADR-024**: 헤드리스 봇의 자동화 방식
