...
> **이 문서와 `/work:review` 슬래시 커맨드의 차이**:
> - 본 체크리스트 = Tier 2 (자동, 매 코드 변경 후, 요약 보고)
> - `/work:review` = Tier 3 (수동, Phase 단위, 상세 보고). 본 체크리스트와 *내용 동일*하되 출력 폭이 다름.
...
| 2B.4 | 코드 작업 응답 끝에 `work-envelope` 봉투 누락 | ADR-018 | 🟡 (reviewer는 보고만, 훅이 강제) |
...
`/work:review` 슬래시 커맨드의 "점검 4" 그대로. 게임 서버 도메인에서 테스트 없으면 늦게 폭발하는 영역은 🔴, 일반 영역은 🟡.
