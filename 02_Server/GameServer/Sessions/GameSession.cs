...
public class GameSession : PacketSession
{
    ...
    // ── 파티 (M5 Phase 04 / M7.6 P03 추출) ────────────────────────────────────
    //
    // 파티 오케스트레이션(거절 규칙·결성·해산·통보 + tick 마샬링)은 PartyFlow로 추출.
    // 여기 남는 건 *세션 auth 게이트*뿐 — `_entityId`/`world` null 게이트(세션이 `_entityId` 소유)
    // 통과 후 PartyFlow에 위임. 행위자 entityId는 *항상* `_entityId`(패킷값 X) — 위장 차단 보존.
