...
namespace Dawnholder.Server.Network
{
    ...
    public abstract class Session
    {
        ...
        protected int m_disconnected = 0; // 연결이 끊어진 상태를 나타내는 플래그 (0: 연결됨, 1: 끊김)

        RecvBuffer m_recvBuffer = new RecvBuffer(65535); // 수신 버퍼

        protected object m_lock = new object(); // 멀티쓰레드 환경에서 큐에 대한 동기화를 위한 락 객체
        protected Queue<ArraySegment<byte>> m_SendQueue = new Queue<ArraySegment<byte>>(); // 전송할 데이터 큐

        protected List<ArraySegment<byte>> m_PendingList = new List<ArraySegment<byte>>(); // 현재 전송이 대기 중인 데이터의 버퍼 리스트
        ...
        public abstract void OnConnected(EndPoint _endPoint); // 연결이 성공적으로 완료되었을 때 호출될 메서드
        public abstract void OnDisconnected(EndPoint _endPoint); // 연결이 끊어졌을 때 호출될 메서드
        public abstract int  OnRecv(ArraySegment<byte> buffer); // 데이터 수신이 완료되었을 때 호출될 메서드
        public abstract void OnSend(int _numOfBytes); // 데이터 전송이 완료되었을 때 호출될 메서드

        void Clear() // 세션 초기화 메서드
        {
            lock (m_lock) // 멀티쓰레드 환경에서 큐에 대한 동기화를 위해 락을 사용
            {
                m_SendQueue.Clear(); // 전송할 데이터 큐 초기화
                m_PendingList.Clear(); // 대기 중인 데이터의 버퍼 리스트 초기화
            }
        }
        ...
        {
            m_Socket = socket; // 클라이언트와의 통신에 사용할 소켓 저장
        ...
        public virtual void Send(ArraySegment<byte> _sendBuff)
        {
            lock (m_lock) // 멀티쓰레드 환경에서 큐에 대한 동기화를 위해 락을 사용
            {
                m_SendQueue.Enqueue(_sendBuff); // 전송할 데이터를 큐에 추가

                if (m_PendingList.Count == 0) // 현재 전송이 대기 중인 데이터가 없다면
        ...
        {
            if(_sendBuffList.Count == 0) // 전송할 데이터가 없는 경우
                return;

            lock (m_lock) // 멀티쓰레드 환경에서 큐에 대한 동기화를 위해 락을 사용
            {
                foreach (ArraySegment<byte> _sendBuff in _sendBuffList)
                    m_SendQueue.Enqueue(_sendBuff); // 전송할 데이터를 큐에 추가
        ...
        {
            if (m_disconnected == 1) // 연결이 끊어진 상태라면 전송 시도하지 않음
                return;

            while (m_SendQueue.Count > 0) // 전송할 데이터가 큐에 남아있는 동안
            {
                ArraySegment<byte> buffer = m_SendQueue.Dequeue(); // 큐에서 전송할 데이터를 하나씩 꺼냄
                m_PendingList.Add(buffer); // 전송할 데이터를 버퍼 리스트에 추가
            }
            ...
            try
            {
                bool pending = m_Socket!.SendAsync(m_SendArgs); // 비동기적으로 데이터 전송 시작

                if (pending == false) // SendAsync가 즉시 완료된 경우 (즉, 전송이 대기 상태가 아닌 경우)
                    OnSendCompleted(null, m_SendArgs); // 전송이 즉시 완료되었으므로 OnSendCompleted를 직접 호출하여 후속 작업 수행
            }
            catch (Exception e)
            {
        ...
        private void OnSendCompleted(object? _sender, SocketAsyncEventArgs _args)
        {
            lock (m_lock) // 멀티쓰레드 환경에서 큐에 대한 동기화를 위해 락을 사용
            {
                ...
                { // 전송이 성공적으로 완료된 경우
                    try
                    {
                        m_SendArgs.BufferList = null; // 전송이 완료된 후 버퍼 리스트 초기화 (굳이 넣을 필요는 없음)
                        m_PendingList.Clear(); // 대기 중인 데이터의 버퍼 리스트 초기화

                        OnSend(m_SendArgs.BytesTransferred); // 완료된 전송 처리

                        if (m_SendQueue.Count > 0) // 큐에 추가로 전송할 데이터가 남아있는 경우
                        {
                            RegisterSend();
        ...
        {
            if (m_disconnected == 1) // 연결이 끊어진 상태라면 수신 시도하지 않음
                return;

            m_recvBuffer.Clean(); // 버퍼 정리
            ArraySegment<byte> segment = m_recvBuffer.WriteSegment; // 수신 버퍼에서 쓸 수 있는 공간의 참조.
            m_RecvArgs.SetBuffer(segment.Array, segment.Offset, segment.Count); // 수신 버퍼 설정

            try
            {
                bool pending = m_Socket!.ReceiveAsync(m_RecvArgs); // 비동기적으로 데이터 수신 시작

                if (pending == false) // ReceiveAsync가 즉시 완료된 경우 (즉, 수신이 대기 상태가 아닌 경우)
                {
                    OnRecvCompleted(null, m_RecvArgs); // 수신이 즉시 완료되었으므로 OnRecvCompleted를 직접 호출하여 후속 작업 수행
                }
            }
            catch (Exception e)
            {
        ...
        private void OnRecvCompleted(object? _sender, SocketAsyncEventArgs _args)
        {
            if (_args.BytesTransferred > 0 && _args.SocketError == SocketError.Success)
            { 
                // 수신이 성공적으로 완료된 경우
                ...
                { 
                    // Write 커서 이동.
                    if (m_recvBuffer.OnWrite(_args.BytesTransferred) == false) // 쓰기 커서를 이동시켰는데, 버퍼가 꽉 찼다면
                    {
                        Disconnect(); // 연결 종료
                        return;
                    }
