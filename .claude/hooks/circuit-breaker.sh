...
# ─────────────────────────────────────────────
# 비파괴적 도구 제외 (정당한 대량 반복 보호)
# ─────────────────────────────────────────────
#   Bash: 테스트 fuzz 1000회·batch 명령 정상 (원 설계).
#   Read/Grep/Glob: 읽기·탐색은 리서치·진단·리팩토링 탐색에서 대량 반복이 *정상*이고,
#     ★ 서브에이전트(Agent 도구)의 Read도 메인 세션 5분 카운트에 합산됨 (2026-06-20 발견,
#       FEATURE_MAP 리서치 fan-out 3개로 count 67 도달 → false-positive trip). 비파괴적이라
#       런어웨이여도 위험 낮음 → 차단 신호 제외. 런어웨이 감시는 *변이* 도구(Edit/Write)에 집중.
case "$TOOL_NAME" in
  Bash|Read|Grep|Glob) exit 0 ;;
esac
