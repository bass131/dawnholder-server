---
description: 새 세션 시작 — git 안전 점검 + work-pin drift 확인 + CHANGELOG 최근 변경 짧게 확인
---
...
work-pin 갱신은 본인이 결정 (자동 갱신 X):
  - `.claude/state/current-pin.txt` "현재 작업 / 다음 액션"을 실제 상태로 반영

해결 후 /session:start 다시 호출해주세요.
```

**절대 금지**: Claude가 work-pin 자동 갱신 X. 사용자가 "그냥 너가 고쳐줘"라 해도 한 단계씩 안내만, 본인이 결정·실행 (학부생 인지 게이트 보호).
...
work-pin(`.claude/state/current-pin.txt`)은 훅이 *매 입력마다 자동 주입*하므로 별도로 읽지 않음 — 이미 컨텍스트 상단에 있음. 톤·사용자 컨텍스트는 `CLAUDE.md`(자동 로드) + memory(자동 로드).

work-pin에서 확인:
- **현재 작업** — 오늘 출발점
- **다음 액션** 줄
- 톤은 `CLAUDE.md` "사용자 컨텍스트" (학부생 멘토링 / trade-off / 솔직함 / 등급별 보고)
...
**판정**:
- work-pin "마지막 갱신" 날짜보다 새로운 [H] 또는 [M] 변경 → 명시적 안내
...
work-pin "현재 작업 / 다음 액션" 따라 **짧게** 응답. 길게 요약 X.
...
work-pin 마지막 갱신 이후 새로운 [H]/[M] 변경 있으면 응답에 한 섹션 추가:
...
- **0단계 게이트는 우회 금지** — 사용자가 "그냥 진행해"라 해도 (B)/(C) 상태에서는 1단계 진입 X. 작업물 유실 위험이 헌법보다 우선
- work-pin "현재 작업"이 비어있거나 오래됐으면 (마지막 갱신 7일 이상) 사용자에게 알리고 재정렬 제안
