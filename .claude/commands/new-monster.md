...
1. `98_Shared/GameData/Tables/monsters.json`을 읽어 기존 스키마 확인 후
   다음 빈 monster id 선택.
...
4. `02_Server/GameServer/Maps/Definitions/<map>.json`의 해당 맵 정의 파일에
   spawn entry 추가.
5. 서버 컨텐츠 로더의 schema 체크가 여전히 통과하는지 확인:
   `dotnet test 02_Server/GameServer.Tests --filter Category=ContentSchema`.
6. 참고: 클라이언트는 `03_Client/Assets/Resources/Content/Monsters/`에
