...
{
    // EnemyRegistry에서 추출한 GameObject 빌더 담당 정적 클래스.
    ...
    public static class EnemyViewFactory
    {
        ...
        static void TryLoadEnemySprites()
        {
            ...
            // BlueBlue 색만 큰 sprite (1.5x1.5) — 나머지 색은 0.23×0.18로 매우 작음.
        ...
        // 런타임 placeholder GameObject 생성. Normal=Mushroom sprite, Boss=ToxicFrog sprite + HP bar 자식.
        // Sprite는 Editor에서 AssetDatabase로 load (Editor only).
        // Sprite asset 없으면 placeholder(흰 사각형 + tint)로 fallback.
                                                   ...
                                                   out float fullWidth)
        {
            ...
            // Boss ToxicFrogBlueBlue (1.5x1.5) → size=2.5 (world 3.75x3.75).
            ...
            //   Mushroom: 그림 발 = sprite bottom (offset 0), 그림 머리 ≈ 1.7
            //   Boss (ToxicFrogBlueBlue): visualFootOffset=-1 (world) → -0.4 (local) 그림 발,
            //     그림 머리 ≈ -0.4 + 1.5 + 여유 0.2 = 1.3 (local) → world = -1 + 1.3*2.5 = 2.25
