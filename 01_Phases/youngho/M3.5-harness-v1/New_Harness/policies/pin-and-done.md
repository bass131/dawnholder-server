...
본 프로젝트에는 *현재 작업 좌표*를 담는 두 자산이 공존합니다:

| 자산 | 위치 | 갱신 빈도 | 누가 읽나 |
|---|---|---|---|
| **work-pin** | `.claude/state/current-pin.txt` | 자주 (매 응답 영향 가능) | AI 매 턴 자동 주입 |
| **CONTEXT.md "⏸️ 현재 멈춤 지점"** | `CONTEXT.md` | 드물게 (마감 시점) | `/session:start`가 통독 |

두 곳이 어긋나면:
- 매 턴 work-pin = 최신 좌표 주입
- `/session:start` 통독 = 옛 멈춤 지점 읽음
- → AI가 *어느 게 진짜*인지 못 잡음 → 옛 결정 기반 작업 위험 (Claude 혼선)

### 정합 약속 (옵션 C)

**두 자산 대등** — work-pin은 *작업 중 좌표*, CONTEXT는 *세션 진입 통독 좌표*. 갱신 시점 다름. **정합은 `/session:end` 단일 게이트에서 단방향 동기 (work-pin → CONTEXT)**.

```
[작업 중]                          [/session:end 호출 시]
work-pin 자주 갱신       →        work-pin 최신 = CONTEXT "현재 멈춤 지점"으로 동기
CONTEXT는 그대로                   (단방향 동기, 정합 보장)
```

### `/session:end` 동기화 룰

| CONTEXT 항목 | 동기 분기 | 트리거 |
|---|---|---|
| **⏸️ 현재 멈춤 지점** | ✅ **등급 무관 항상 동기** | work-pin 압축본 → CONTEXT 풀어쓴 한 문단으로 박음 |
| **학습 일지 후보** | ✅ 콘텐츠 유무 분기 | -DONE.md / work-pin에 학습 키워드 *실제 있으면* 추가 (★ 평가 포함). 키워드 없으면 스킵 |
| **CONTEXT_History.md** | ✅ 매 마감 한 줄 자동 | 비용 0, 이력 보존 |
| **응축 평가** | ⚠️ 250줄+ 알림만 | 자동 응축 X. 마일스톤 끝 사용자 명시 결정 |
| **다른 섹션** (하드 일정/팀 구조/보류 중/핵심 결정) | ❌ 자동 갱신 X | 사용자 의도 보호 |

### 핵심 정신: 등급 무관 "현재 멈춤 지점" 동기

옛 운영은 "단순/보통 등급 = CONTEXT 갱신 스킵" 분기 고려했으나, **work-pin과 정합이 깨지면 양식 부담 ↓보다 Claude 혼선 비용 ↑**. 따라서:

- **등급은 *갱신 깊이/콘텐츠*만 분기** (학습 후보 콘텐츠 유무 등)
- **"현재 멈춤 지점" = 항상 동기** (등급 무관, work-pin 정합 보장이 양식 부담보다 우선)
- 양식 부담은 미리보기 컨펌 *기본 OK 디폴트*로 줄임 (사용자가 "스킵" 명시할 때만 스킵)
...
- 한 쪽이라도 not ignored → STOP + 알림 (협업 셋업 어긋남 — 본인 자산은 각자 보유)

### 함정

- **work-pin 갱신 시 CONTEXT 동시 갱신 X** — 작업 중엔 work-pin만 갱신 (정합은 `/session:end`에서만). 작업 중 매번 두 곳 동기 = 양식 부담 ↑↑
- **CONTEXT가 work-pin 추출 자료 X** — work-pin은 *압축 좌표*, CONTEXT는 *맥락 포함 한 문단*. 단순 복사 X
- **마일스톤 끝엔 응축 평가 의무** — 250줄+ 임계 도달 시 알림만 — 본인이 *결정*해서 처음부터 재작성. 자동 응축 X (옛 사고 위험)
