...
//   BossRoom 진입에는 40킬 게이트(Q3)가 걸린다. 이 시나리오는 4맵 루프 *전환 메커니즘*
...
//     killCount 전제조건을 충족 → 게이트는 서버 권위 카운트(40)를 읽어 정당 통과.
...
public class MapTransitionScenario
{
    ...
    /// 표준 봇 런(null)은 killCount=0이므로 40킬 게이트(Q3)에 막힌다.
    /// 게이트 e2e는 BossPortalGateTests(stub) + R2 BossGateSmoke(별도)가 담당한다.
    /// </param>
    ...
    sealed class TransitionProbe
    {
        ...
        // 서버 권위 X 좌표 — S_Snapshot.x로 실시간 갱신 (network thread 쓰기 / scenario thread 읽기).
        // float volatile: C# spec §14.5.4 — float은 word-tearing 없으므로 volatile 단독으로 충분.
        // 초기값은 S_EnterMap / S_MapTransition의 spawnX로 동기화.
        volatile float _serverX;

        // MoveToPortal 루프 최대 반복 상한 (400틱 ≈ 20s @ 50ms/tick — 진짜 막힌 경우 loud 실패).
        ...
        // 서버 권위 X(_serverX)를 매 틱 확인해 방향을 재계산하므로 HitState(넉백/이동 잠금) 후에도 수렴.
        // 루프 상한(MoveToPortalMaxTicks)에 도달하면 TimeoutException으로 loud 실패.
        ...
        void HandlePacket(ArraySegment<byte> buffer)
        {
            ...
            switch ((PacketID)id)
            {
                ...
                case PacketID.S_EnterMap:
                    ...
                    _serverX = enterMap.spawnX; // S_Snapshot 추적 초기화 — spawn 좌표로 동기화.
                ...
                case PacketID.S_MapTransition:
                    ...
                    _serverX = transition.spawnX; // 새 맵 spawn 좌표로 재동기화.
                ...
                case PacketID.S_Snapshot:
                    // 서버 권위 위치 실시간 추적 — MoveToPortal robust화에 사용.
                    // entityId 필터: 내 엔티티 snapshot만 적용 (멀티플레이어 환경 대비).
                    // volatile 쓰기 — network thread에서 쓰고 scenario thread가 읽음.
                ...
                // 이 시나리오는 전투 피격을 허용하나 S_HitResult 등 전투 패킷은 무시.
                // MoveToPortal이 _serverX 추적으로 HitState 후 자연 수렴.
            }
        }
